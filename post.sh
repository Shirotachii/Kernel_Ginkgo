...
KBUILD_COMPILER_STRING=$(/home/neko/clang/bin/clang --version | head -n 1 | perl -pe 's/\(http.*?\)//gs' | sed -e 's/  */ /g' -e 's/[[:space:]]*$//')
...
function copy() {
    for files in "$KERNEL_IMG" "$KERNEL_DTBO"; do
        if [ -f "$files" ]; then
            echo -e " Copy [$files] to AnyKernel3 directory"
            cp "$files" "$AK3_DIR"
        else
            echo -e ""
            echo -e " Image/dtbo is missing!"
            echo -e ""
            exit 1
        fi
    done
